...
func AddOrder(order Order) {
	...
	sqlStatement := "INSERT INTO orders (order_id, order_name, start_date) VALUES ($1, $2, $3)"

	_, err = db.Exec(sqlStatement, order.Order_id, order.Order_name, order.Start_date)
	if err != nil {
		log.Fatal(err)
	}
}
...
func AddTask(task Task, id int) {
	...
	sqlStatement := "INSERT INTO tasks (task_id, task_name, duration, resource, prev_tasks, order_id) VALUES ($1, $2, $3, $4, $5, $6)"

	_, err = db.Exec(sqlStatement, task.Task_id, task.Task_name, task.Duration, task.Resource, task.Prev_work, id)
	if err != nil {
		log.Fatal(err)
	}
}
